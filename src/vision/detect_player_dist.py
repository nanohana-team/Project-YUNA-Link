...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Detect player distance from stereo VR window using screen-region capture"
    )
    ...
    while True:
        ...
        if time.time() - last_heartbeat >= args.heartbeat_sec:
            ...
            log(
                ...
                f"last_window={last_window_name!r}"
            )
        ...
        try:
            ...
            log(
                f"[TRACE] frame.shape={frame.shape} mean={frame_mean:.2f} "
                f"std={frame_std:.2f} diff={frame_diff_mean:.3f}"
            )
